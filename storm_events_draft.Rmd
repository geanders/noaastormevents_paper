...
Several organizations currently collect data in attempts to compile, categorize, and quanitfy weather events and their impacts. Some of these organizations include the National Weather Service, the National Center for Environmental Information (NCEI), and the Federal Emergency Management Agency (FEMA). Data collection improves scientits' ability to predict weather events and understand their impacts. This is commonly accomplished with interdisciplinary research involving various fields of study such as epidemiology, atmospheric science, agriculture, economics, etc. This research is critical to help avoid such large fallout and loss from weather events in the future. 

Because of this, it is incumbent that scientists understand the biases and limitations in weather datasets to use the large amounts of data appropriately and portray new findings accurately.  Certain biases and limitations arise in these datasets because storing disaster data is a challenging endeavor. Challenges arise as the process of recording disaster data and the technology we use to do it are constantly evolving. Gall et al. recognized this issue and wrote a paper covering six bias types associated with major disaster databases used in the US [@gall2009losses]. 

The Gall et. al. paper briefly covers how these bias types affect the NOAA Storm Events database which we will expand on in this paper. The NOAA Storm Events database is maintained, in part, by the National Weather Service. It currently contains information on 55 different storms, significant weather phenomena, rare/unusual weather, and other significant meteorological events across the United States. NOAA has been recording weather information since 1950 and storing it files for each year. NOAA has changed its recording strategies several times from only recording tornado events starting in 1950 to its current 55 event types. Following these changes, they have reformatted and standardized events types without changing any specific values or details [@stormeventsdetails]. 

Despite these standardizations, bias arises within the NOAA Storm events data due to factors including structural changes over time, reporting errors, inherent bias, and others. In our paper, we will examine the NOAA Storm Events database and investigate the presence of mechanisms that lead to major biases in the data. The major biases are separated into five categories of hazard bias, temporal bias, threshold bias, accounting bias, and geographic bias. 
...
In the context of this paper, we use the term hazard to refer to all of the possible major storm event types that can be entered in the NOAA Storm Events database. We use the term disaster to refer to a reported occurence of a specific hazard. NOAA Storm Events currently records 55 different event types that are listed in the NWS instruction manual [@nwsinstruction]. Most of these event types correlate to one type of hazard but some of them correlate to different intensitions of the same type of hazard. Some examples of this include the event type "Strong Wind" versus "High Wind" or "Cold/Wind Chill" versus "Extreme Cold/Wind Chill". In these instances, the hazard would be wind or wind chill while the intensities are strong, high, cold, or extreme cold. 
...
* think of intention when trying to assess exposure, do you mean to catch every physical instance of hazard? for some events that isnt even the goal of the database 
