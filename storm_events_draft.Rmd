...
A hazard is something that has the potential to cause damage. It is a threat that can severely disrupt society's workings. Hazards' potential to cause harm are compounded by human factors and human activity. Human factors include culture, gender, race, socioeconomic status and political structure, while human activity includes land development, urbanization, emergency preparedness, response plans, etc [cutter1996responses].  These factors can influence the actual potential for harm, the perceived potential for harm, and the ability to respond if a disaster does occur.  
...
Susan Cutter has done extensive research on hazards and has spent much time defining these different terms. In her work, she states that hazards include the probability of an event occuring (risk), the impact, and the contextual/socioeconomic elements surrounding it.

Other scientific authors have similar thoughts on the term hazard and believe it is important to include human culpability [@mcentire2005revisiting]. 

In the context of this paper, a hazard would be all of the possible storm event types that can be entered in the NOAA Storm Events database. A disaster would be the atual occurence and recording of each hazard type. 


### defining event types 

NOAA Storm Events currently records 55 different event types that are listed in the NWS instruction manual [@nwsinstruction]. 

In 2019, 51 total events were reported, of 55 different event types. The most frequent type of events reported were Thunderstorm Wind, which had over twice as many reports as the second most frequent, Hail, which in turn had almost twice as many events reported as the next most common few---Flood, Flash Flood, and Winter Weather. The least common reported events included Volcanic Ashfall, Sleet, Dense Smoke, Sneakerwave, Seiche, and Marine Tropical Depression.


NOAA has different event names for different intensities of the same type of hazard. Some examples of this include the event type "Strong Wind" versus "High Wind" or "Cold/Wind Chill" versus "Extreme Cold/Wind Chill". 
