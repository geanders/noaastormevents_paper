...
* it is critical to learn more about disasters but the data is tricky
* it is important to learn the quirks about disaster databases
* we need tools to capture exposure and impact data accurately
* mention how Gall's paper on losses covers these topics but we are focusing on Storm Events database 
* make it clear that we are using this database in R ] 
...
Several organizations collect disaster data in the US including the National Weather Service, NCDC, FEMA. Collecting disaster data is very important to understand how hazards impact human life, agriculture, commerce, etc. However, current collections of disaster data posses several flaws and biases. The data can often be varied and vague. 
...
Some of the other parts you've identified here could move into the funnel. For example, 
how collecting disaster data is important for us to improve our understanding of these
impacts, and then this idea that we have large sets of the data, but it comes with quirks
that need to be understood by the researcher to be properly used.]

### Funnel 

Using disaster data is common in interdisciplinary research because natural hazards impact several fields of study: economics, epidemiology, atmospheric science, agriculture, etc. 

[TK: Include examples of studies that use the NOAA Storm Events database here] 

[BA: Yes, what you have here will be great for the funnel. See my note for the opening about
how some of that could move into this funnel, too, so we have the opening more broad 
(focus on disaster impacts, and not get into the question of data until the funnel.)]

### Challenge 

Gall et al. covers six major biases associated with the major disaster databases used in the US [@gall2009losses]. In this paper, we will describe the structure and documentation of NOAA Storm Events and bias can affect its data. We introduce examples of how certain hazards exemplify the biases and how features of the noaastormevents package in R dissect these issues. It is important to understand the biases in Storm Events to accurately portray new findings and avoid making the same mistakes. 
...
* not all events may be reported, some may be double counted
* lots of sources 
* inherent bias in data collection in this specific field 
...
The probability that an event is recorded might differ by event type.   The US government has certain programs that focus on some weather events more than others.  For example, the NWS is obliged to provide monetary loss estimates for any flood event even if the damage assessment is a 'guesstimate' (Gall, Borden, and Cutter 2009).  This leads to overrepresentation of flood events in Storm Data because of how they get information.  [TK: I need to look into the flood insurance program to see if this also has an effect]   

Additionally, certain characteristics of event types influence how easy or difficult it is to obtain information about the event.  It is more difficult to obtain information on event types that do not cause much physical damage or monetary loss. This is because insurance companies typically keep strict records of monetary damages caused by weather events.  If this is lacking, then the NWS struggles to find exact damage amounts.  Drought hazards exemplify this issue; they are notoriously underreported because there is a lack of physical damage and it is hard to quantify spatial and monetary losses (Gall, Borden, and Cutter 2009).  Avalanches also showcase this issue as they typically occur in remote locations with little physical damage or fatalities. [TK: I just made this assumption, so I need to check the literature] On the other hand, hurricanes or tornadoes that pass through large cities will cause high amounts of physical damage and fatalities. This would attract the attention of insurance companies and produce clear records of damages.   
...
Even when storms are entered into the database, all of their associated hazards might not be entered or their events may be double counted.  NOAA records one large event as an episode and all of its associated hazards as events under than episode.   The NWS documentation goes into detail on this recording structure. 
